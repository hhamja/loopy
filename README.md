...
- **The gate hooks are a backstop, not a sandbox.** `decision_gate.sh` and `verifier_guard.sh` catch the *common literal forms* of a T2 / write command from a **cooperative** agent that merely forgot the doctrine — that is their job, and for it they work. They are **not** containment against an adversarial or misaligned agent, which can bypass them: reach a gated action through `git -C … push`, `gh api …/merges`, an env-var prefix (`GIT_DIR=… git push`), `eval` / `sh -c`, a `+main` refspec, or an interpreter/redirect write (`python -c`, `echo … >`). `tamper_gate.sh` (PreToolUse on `Edit|Write|NotebookEdit`) raises an Edit/Write of a verifier-input or gate path — `tests/`, `.github/workflows/`, `scripts/`, `hooks/`, `.claude/loop/rubric.md`, and the `.gate-approved` marker — to a T2 diff-path backstop, closing the honest-but-forgetful "just edit the failing test" path and the Write-tool marker forgery; but a Bash redirect or interpreter still reaches those same paths, so it too is a backstop, not containment. In an **interactive** session the real gate is Claude Code's permission prompt; in **headless/autonomous** runs that prompt is absent, so treat these hooks as defense-in-depth only and rely on the `sandbox` primitive (or a container / worktree with scoped tokens) for true isolation. Chasing each bypass with more regex does not bottom out — the trust boundary must be the sandbox, not the pattern.
