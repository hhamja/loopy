...
- **게이트 훅은 백스톱이지 샌드박스가 아닙니다.** `decision_gate.sh`·`verifier_guard.sh`는 doctrine을 *잊은* **협조적** 에이전트가 내는 T2/쓰기 명령의 *흔한 문자 형태*를 잡습니다 — 그 목적으로는 동작합니다. 하지만 **적대적·오정렬** 에이전트에 대한 봉쇄는 아니며, 다음으로 우회됩니다: (게이트되지 않는) Write 툴로 `.claude/loop/.gate-approved` 위조, `git -C … push`, `gh api …/merges`, 환경변수 접두(`GIT_DIR=… git push`), `eval`/`sh -c`, `+main` refspec, 인터프리터 쓰기(`python -c`, `node -e`). **대화형** 세션의 진짜 게이트는 Claude Code 권한 프롬프트이고, **headless/자율** 실행에선 그 프롬프트가 없으므로 이 훅들은 심층방어로만 취급하고 진짜 격리는 `sandbox` 프리미티브(또는 스코프된 토큰의 컨테이너/worktree)에 의존하십시오. 정규식으로 우회를 하나씩 쫓는 것은 바닥이 없습니다 — 신뢰 경계는 패턴이 아니라 샌드박스여야 합니다.
