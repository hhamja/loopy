...
If `.claude/loop/` does not exist: say so and point to `/loop-harness:loop-init`. Otherwise:
...
4. Suggest the next step: address the top gap, or resume with `/loop-harness:loop-run`.
