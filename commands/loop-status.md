...
If `.claude/loop/` does not exist: say so and point to `/loop-harness:loop-init`. Otherwise read `state.md`, `rubric.md`, `loop.config.md`, and `.claude/loop/.last-usage` (if present), then print:
