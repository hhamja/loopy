...
- **Success — green gate before "done":** every rubric criterion passes → do NOT stop yet. Run the green gate ONCE (full runs only; never for `--once` or `--verify-only`): (a) spawn the `auditor` subagent ("audit this loop's process and decision-gate adherence"), and (b) run the `/code-review` skill on the run's accumulated diff. If either surfaces a real defect, fold it into `rubric.md` as a new/reopened criterion and into `state.md` unresolved, then keep looping under the same rails — a rubric that is green but fails audit or review is not done. When the rubric passes AND the green gate is clean: overwrite `review.md` with the human-review summary, set `loop_active: false` and `human_gate: ready_for_merge`, report done. Say explicitly: "done" is a claim — list what the human should verify by hand before merging.
- **Safety rails (mandatory, never skip):**
  ...
  - the same criterion has failed 3 consecutive cycles → do NOT escalate yet. Replan first per `references/replan.md`: try up to `replan_max` (loop.config.md, default 2) genuinely different strategies (change approach / decompose the work / spike the cause), **never weakening the criterion**. Only if replan is exhausted, escalate with 2-3 concrete options. Interactive: ask the user. Headless: record the options and stall reason in state.md, set `human_gate: stalled`, end the turn.
